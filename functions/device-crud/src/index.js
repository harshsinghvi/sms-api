...
module.exports = async function (req, res) {
  ...
  switch (event) {
    case "create":
      ...
      const { enabled } = await databases.updateCollection
      (BASE_DATABASE_ID, deviceId, `device_${deviceId}`, [], false, true);

      if (!enabled) res.send("Something went Wrong", 500);

      await Promise.all([
        databases.createStringAttribute(BASE_DATABASE_ID, deviceId, "text", 160, true),
        databases.createStringAttribute(BASE_DATABASE_ID, deviceId, "to", 11, false, null, true),
